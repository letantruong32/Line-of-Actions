...
public class BoardTest {
    ...
    /** Test secretArea(). */
    @Test
    public void testSecretArea() {
        Board b1 = new Board(BOARD1, BP);
        Board b2 = new Board(BOARD2, BP);

        assertEquals(5, b2.numSecret(WP));
        assertEquals(6, b2.numSecret(BP));
    ...
    /** Test numContig. */
    @Test
    public void numContigTest() {
        Board b = new Board(BOARD1, BP);
        Square s = sq("a2");
        boolean [][] visited = new boolean[BOARD_SIZE][BOARD_SIZE];
        assertEquals(5, b.numContig(s, visited, WP));

        Board b2 = new Board(BOARD2, BP);
        Square s2 = sq("b5");
        boolean [][] visited2 = new boolean[BOARD_SIZE][BOARD_SIZE];
        assertEquals(8, b.numContig(s2, visited2, WP));
    }

    /** Test computeRegions. */
    @Test
    public void computeRegsTest() {
        Board b = new Board(BOARD4, BP);
        b.computeRegions();
        assertEquals(true, b.gameOver());
        assertEquals(WP, b.winner());
    }
    ...
    /** Test Squares class adjacent*/
    @Test
    public void testSquares() {
        Board b = new Board(BOARD1, BP);
        Square s = sq("f3");
        Square[] adj = adjacent(s);
        assertEquals(8, adj.length);


        Square s2 = sq("a1");
        Square[] adj2 = adjacent(s2);
        assertEquals(3 , adj2.length);
    }
    ...
    /** Test countLOA. */
    @Test
    public void testCountLOA() {
        Board b = new Board(BOARD1, BP);
        Square fr = mv("f3-f8").getFrom();
        Square to = mv("f3-f8").getTo();
        assertEquals(4, b.countLOA(fr, to));

        fr = mv("f3-e4").getFrom();
        to = mv("f3-e4").getTo();
        assertEquals(2, b.countLOA(fr, to));

        fr = mv("c4-c7").getFrom();
        to = mv("c4-c7").getTo();
        assertEquals(3, b.countLOA(fr, to));
    }


    /** Test squares and Move. */
    @Test
    public void testSquaresMovement() {
        Board b = new Board(BOARD1, BP);
        Square fr = mv("f3-h3").getFrom();
        Square to = mv("f3-h3").getTo();
        int dir = fr.direction(to);
        int dist = fr.distance(to);
